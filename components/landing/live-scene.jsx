  ...
  useEffect(() => {
    ...
    const handle = (event) => {
      ...
      if (data.type === "action-result") {
        setAction(null);
        setNotice(data.ok ? { overview: "Whole structure in view.", ligand: "Ligand in focus.", surface: "Molecular surface shown." }[data.action] || "" : "That view could not be shown. Try resetting the scene.");
      }
    };
  ...
  return (
    <div ref={root} id={id} className={`live-scene live-scene-${scene}`} data-status={status}>
      ...
      <div className="live-scene-viewport">
        <ThemedImage light={light} dark={dark} alt={label} width={1804} height={1262} className="live-poster" sizes="(min-width: 1200px) 1200px, 94vw" priority={autoLoad} />
        {mounted && <iframe
          ref={frame}
          src={`/live/${scene}?theme=${theme}`}
