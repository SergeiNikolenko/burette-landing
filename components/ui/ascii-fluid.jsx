 ...
 */
export function createAsciiFluid(canvas, initial = {}) {
  let options = {
  ...
  const clearFbo = (fbo, r = 0, g = 0, b = 0) => {
  ...
  const tick = (now) => {
    ...
    const paper = hexToRgb(p.backgroundColor ?? (dark ? DARK.paper : LIGHT.paper))
    ...
    if (p.interactive && mouse.moved && mouse.inside && !reduce) {
      ...
      gl.uniform2f(gl.getUniformLocation(splat.program, "u_point"), mouse.x, mouse.y)
      gl.uniform3f(
      ...
      gl.uniform1i(gl.getUniformLocation(splat.program, "u_target"), 0)
      gl.uniform2f(gl.getUniformLocation(splat.program, "u_point"), mouse.x, mouse.y)
      gl.uniform3f(gl.getUniformLocation(splat.program, "u_color"), dyeAmt, 0, 0)
      gl.uniform1f(gl.getUniformLocation(splat.program, "u_radius"), brushR * 1.15)
      gl.uniform1f(gl.getUniformLocation(splat.program, "u_aspect"), aspect)
    ...
    if (!reduce) {
      ...
      gl.uniform2f(gl.getUniformLocation(advect.program, "u_texel"), texel[0], texel[1])
      ...
      gl.uniform2f(gl.getUniformLocation(divergence.program, "u_texel"), texel[0], texel[1])
      blit(divergenceFbo)
      ...
      for (let i = 0; i < 14; i++) {
        ...
        gl.uniform2f(gl.getUniformLocation(pressure.program, "u_texel"), texel[0], texel[1])
      ...
      gl.uniform2f(gl.getUniformLocation(gradient.program, "u_texel"), texel[0], texel[1])
      blit(velocity.write)
      velocity.swap()
    ...
    gl.uniform1f(gl.getUniformLocation(display.program, "u_charCount"), atlas.count)
    gl.uniform3f(gl.getUniformLocation(display.program, "u_ink"), ink[0], ink[1], ink[2])
    gl.uniform3f(
